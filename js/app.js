...
const util = (() => {
    ...
    const tamu = () => {
        ...
        let pronoun = (new URLSearchParams(window.location.search)).get('p');
        ...
        // if pronoun is not empty and not null
        if (pronoun) {
        let div = document.createElement('div');
        div.classList.add('m-2');
        div.innerHTML = `<p class="mt-0 mb-1 mx-0 p-0 text-light">Kepada Yth </p><h2 class="text-light">${escapeHtml(pronoun)} ${escapeHtml(name)}</h2>`;
